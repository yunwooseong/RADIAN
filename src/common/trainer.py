...
from utils.utils import dict2str, early_stopping, get_local_time
...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        total_loss = None
        loss_batches = []
        for batch_idx, interaction in enumerate(tqdm(train_data)):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if saved:
                    if valid_score >= self.best_valid_score:
                        ...
                        if verbose:
                            self.logger.info("Model saved at epoch %d" % epoch_idx)
    ...
    def evaluate(self, eval_data, is_test=False, idx=0):
        ...
        # batch full users
        batch_matrix_list = []
        for batch_idx, batched_data in enumerate(tqdm(eval_data)):
    ...
    def _save_checkpoint(self, model):
        ...
        save_path = log_directory(self.config)
        hp_str = ""
        if "hyper_parameters" in self.config:
            for param_name in self.config["hyper_parameters"]:
                hp_str += f"_{param_name}{self.config[param_name]}"
        model_path = os.path.join(save_path, f"Best_{self.config['model']}{hp_str}.pth")
