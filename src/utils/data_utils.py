...
def mask_batch_text_tokens(inputs, tokenizer, mlm_probability=0.15, is_train=True):
    ...
    random_words = torch.randint(len(tokenizer), labels.shape, dtype=torch.long)  # len(tokenizer) == #vocab
    ...
    def __call__(self, img):
        ...
        if isinstance(img, torch.Tensor):
            ...
            return img_tensor_resize(
                img, size=get_resize_size(img, self.max_size), mode=self.interpolation, align_corners=False
            )
...
def get_imagenet_transform(min_size=600, max_size=1000):
    ...
    if min_size != 600:
        ...
        warnings.warn(f"Warning: min_size is not used in image transform, " f"setting min_size will have no effect.")
...
def repeat_tensor_rows(raw_tensor, row_repeats):
    ...
    if sum(row_repeats) == len(row_repeats):
        ...
        indices = torch.LongTensor(flat_list_of_lists([[i] * r for i, r in enumerate(row_repeats)])).to(
            raw_tensor.device
        )
